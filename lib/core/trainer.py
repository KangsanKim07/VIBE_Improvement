...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            start = time.time()
            inp2 = torch.zeros((inp.shape[0], inp.shape[1]+1, inp.shape[2]))
            inp2[:, 1:, :] = inp
            inp2[:, 0, :] = torch.mean(inp, 1)
            preds = self.generator(inp2.cuda())
    ...
    def validate(self):
        ...
        for i, target in enumerate(self.valid_loader):
            ...
            with torch.no_grad():
                ...
                inp2[:, 1:, :] = inp
                inp2[:, 0, :] = torch.mean(inp, 1)
                preds = self.generator(inp2.cuda(), J_regressor=J_regressor)
